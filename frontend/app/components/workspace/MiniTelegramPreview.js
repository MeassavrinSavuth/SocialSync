...
import { FaThumbsUp, FaCommentAlt, FaShare, FaGlobeAmericas, FaEllipsisH, FaTelegramPlane } from 'react-icons/fa';

export default function MiniTelegramPreview({ task, onReact, showReactions = true, showTitle = false, fullWidth = false, onEdit, onPost, onDelete, fitMode = 'auto', workspaceId, canEdit = true, canDelete = true, canPublish = true }) {
  ...
  return (
    <div className={`bg-white rounded-xl shadow border border-cyan-200 ring-1 ring-cyan-100 ${fullWidth ? 'w-full' : 'w-full max-w-lg mx-auto'}`}>
      {/* Header */}
      <div className="flex items-center px-4 pt-4 pb-2 justify-between relative">
        <div className="flex items-center">
          <div className="w-11 h-11 bg-gradient-to-br from-blue-400 to-blue-500 rounded-full flex items-center justify-center text-white font-bold text-sm mr-3">
            <FaTelegramPlane />
          </div>
